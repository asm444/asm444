...
<sub align="center">Generated weekly by <a href="./.github/workflows/metrics.yml">a GitHub Action</a> that counts artifacts in the repository trees. No third-party service, and the queries run unauthenticated on purpose — every bar is a number you can reproduce yourself.</sub>

## Beyond these repositories

Four of my pull requests were merged into
[MrSchrodingers/claude-usage-widget](https://github.com/MrSchrodingers/claude-usage-widget?q=is%3Apr+author%3Aasm444):
Firefox cookie handling, Chrome fallback with installer health checks, Ubuntu
compatibility with strict error reporting, and cross-platform notifications.

Most of my work, however, happens in private repositories — the contribution graph on my
profile includes it. There I build LLM-based document triage for the legal domain:
multimodal classification, role-based permissioning, and evidence-driven test auditing.
Those commits are real but not linkable, so I keep them out of the chart above: every bar
there is something you can click and count yourself.
